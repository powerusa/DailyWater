package com.watercamel.util

import java.util.Locale

/** In-app localization helper. Returns translated strings for the given language code. */
object L10n {

    data class LangOption(val code: String, val name: String)

    val supportedLanguages = listOf(
        LangOption("device", "📱 Device"),
        LangOption("en", "English"),
        LangOption("es", "Español"),
        LangOption("zh", "中文"),
        LangOption("de", "Deutsch"),
        LangOption("pl", "Polski"),
        LangOption("pt", "Português")
    )

    fun t(key: String, lang: String): String {
        val effective = resolve(lang)
        return translations[effective]?.get(key) ?: translations["en"]?.get(key) ?: key
    }

    fun dayStreak(count: Int, lang: String): String {
        val fmt = t("day_streak_fmt", lang)
        return String.format(fmt, count)
    }

    fun resolve(lang: String): String {
        if (lang != "device") return lang
        val code = Locale.getDefault().language
        return if (translations.containsKey(code)) code else "en"
    }

    private val translations: Map<String, Map<String, String>> by lazy {
        mapOf("en" to en, "es" to es, "zh" to zh, "de" to de, "pl" to pl, "pt" to pt)
    }

    private val en = mapOf(
        "app_name" to "Water Camel",
        "goal_reached" to "Goal reached 🎉",
        "set_your_daily_goal" to "Set Your Daily Goal",
        "set_goal_prompt" to "Tap the gear icon to set a daily water goal\nand start tracking.",
        "set_goal" to "Set Goal",
        "day_streak_fmt" to "%d-day streak!",
        "today" to "Today",
        "goal" to "Goal",
        "remaining" to "Remaining",
        "todays_timing" to "Today's Timing",
        "morning" to "Morning",
        "afternoon" to "Afternoon",
        "evening" to "Evening",
        "weekly_monthly_charts" to "📊 Weekly & Monthly Charts",
        "quick_add" to "Quick Add",
        "custom_amount" to "Custom Amount",
        "undo" to "Undo",
        "reset" to "Reset",
        "reset_today" to "Reset Today?",
        "reset_today_msg" to "This will clear all intake for today.",
        "cancel" to "Cancel",
        "settings" to "Settings",
        "daily_goal" to "Daily Goal",
        "enter_goal" to "Enter goal",
        "unit" to "Unit",
        "bottle" to "\uD83C\uDF76 Bottle",
        "bottle_setup" to "Bottle Setup",
        "bottle_size_label" to "1 bottle = ? oz",
        "bottle_hint" to "Quick-add will show bottle fractions (¼, ½, ¾, 1).",
        "appearance" to "Appearance",
        "dark_mode" to "Dark Mode",
        "language" to "Language",
        "save" to "Save",
        "history" to "History",
        "keep_it_going" to "Keep it going!",
        "recent_days" to "Recent Days",
        "goal_met" to "Goal met",
        "done" to "Done",
        "trends" to "Trends",
        "seven_days" to "7 Days",
        "fourteen_days" to "14 Days",
        "thirty_days" to "30 Days",
        "average" to "Average",
        "goals_met" to "Goals Met",
        "streak" to "Streak",
        "daily_intake" to "Daily Intake",
        "goal_line" to "Goal line",
        "back" to "Back",
        "bottles" to "bottles",
        "goal_colon" to "Goal:"
    )

    private val es = mapOf(
        "app_name" to "Water Camel",
        "goal_reached" to "¡Meta alcanzada 🎉",
        "set_your_daily_goal" to "Establece tu meta diaria",
        "set_goal_prompt" to "Toca el ícono de engranaje para establecer una meta diaria de agua.",
        "set_goal" to "Establecer meta",
        "day_streak_fmt" to "¡Racha de %d días!",
        "today" to "Hoy",
        "goal" to "Meta",
        "remaining" to "Restante",
        "todays_timing" to "Horario de hoy",
        "morning" to "Mañana",
        "afternoon" to "Tarde",
        "evening" to "Noche",
        "weekly_monthly_charts" to "📊 Gráficos semanales y mensuales",
        "quick_add" to "Agregar rápido",
        "custom_amount" to "Cantidad personalizada",
        "undo" to "Deshacer",
        "reset" to "Reiniciar",
        "reset_today" to "¿Reiniciar hoy?",
        "reset_today_msg" to "Esto borrará toda la ingesta de hoy.",
        "cancel" to "Cancelar",
        "settings" to "Ajustes",
        "daily_goal" to "Meta diaria",
        "enter_goal" to "Ingresa la meta",
        "unit" to "Unidad",
        "bottle" to "\uD83C\uDF76 Botella",
        "bottle_setup" to "Configuración de botella",
        "bottle_size_label" to "1 botella = ? oz",
        "bottle_hint" to "Agregar rápido mostrará fracciones de botella (¼, ½, ¾, 1).",
        "appearance" to "Apariencia",
        "dark_mode" to "Modo oscuro",
        "language" to "Idioma",
        "save" to "Guardar",
        "history" to "Historial",
        "keep_it_going" to "¡Sigue así!",
        "recent_days" to "Días recientes",
        "goal_met" to "Meta cumplida",
        "done" to "Listo",
        "trends" to "Tendencias",
        "seven_days" to "7 días",
        "fourteen_days" to "14 días",
        "thirty_days" to "30 días",
        "average" to "Promedio",
        "goals_met" to "Metas cumplidas",
        "streak" to "Racha",
        "daily_intake" to "Ingesta diaria",
        "goal_line" to "Línea de meta",
        "back" to "Atrás",
        "bottles" to "botellas",
        "goal_colon" to "Meta:"
    )

    private val zh = mapOf(
        "app_name" to "Water Camel",
        "goal_reached" to "目标达成 🎉",
        "set_your_daily_goal" to "设定每日目标",
        "set_goal_prompt" to "点击齿轮图标设定每日饮水目标并开始记录。",
        "set_goal" to "设定目标",
        "day_streak_fmt" to "连续%d天！",
        "today" to "今日",
        "goal" to "目标",
        "remaining" to "剩余",
        "todays_timing" to "今日时段",
        "morning" to "早晨",
        "afternoon" to "下午",
        "evening" to "晚上",
        "weekly_monthly_charts" to "📊 每周和每月图表",
        "quick_add" to "快速添加",
        "custom_amount" to "自定义量",
        "undo" to "撤销",
        "reset" to "重置",
        "reset_today" to "重置今天？",
        "reset_today_msg" to "这将清除今天的所有摄入记录。",
        "cancel" to "取消",
        "settings" to "设置",
        "daily_goal" to "每日目标",
        "enter_goal" to "输入目标",
        "unit" to "单位",
        "bottle" to "\uD83C\uDF76 水瓶",
        "bottle_setup" to "水瓶设置",
        "bottle_size_label" to "1瓶 = ? oz",
        "bottle_hint" to "快速添加将显示水瓶分数（¼、½、¾、1）。",
        "appearance" to "外观",
        "dark_mode" to "深色模式",
        "language" to "语言",
        "save" to "保存",
        "history" to "历史",
        "keep_it_going" to "继续保持！",
        "recent_days" to "最近几天",
        "goal_met" to "达标",
        "done" to "完成",
        "trends" to "趋势",
        "seven_days" to "7天",
        "fourteen_days" to "14天",
        "thirty_days" to "30天",
        "average" to "平均",
        "goals_met" to "达标天数",
        "streak" to "连续",
        "daily_intake" to "每日摄入",
        "goal_line" to "目标线",
        "back" to "返回",
        "bottles" to "瓶",
        "goal_colon" to "目标:"
    )

    private val de = mapOf(
        "app_name" to "Water Camel",
        "goal_reached" to "Ziel erreicht 🎉",
        "set_your_daily_goal" to "Tagesziel festlegen",
        "set_goal_prompt" to "Tippe auf das Zahnrad, um ein tägliches Wasserziel festzulegen.",
        "set_goal" to "Ziel festlegen",
        "day_streak_fmt" to "%d-Tage-Serie!",
        "today" to "Heute",
        "goal" to "Ziel",
        "remaining" to "Verbleibend",
        "todays_timing" to "Heutiges Timing",
        "morning" to "Morgen",
        "afternoon" to "Nachmittag",
        "evening" to "Abend",
        "weekly_monthly_charts" to "📊 Wochen- & Monatsdiagramme",
        "quick_add" to "Schnell hinzufügen",
        "custom_amount" to "Eigene Menge",
        "undo" to "Rückgängig",
        "reset" to "Zurücksetzen",
        "reset_today" to "Heute zurücksetzen?",
        "reset_today_msg" to "Alle heutigen Einträge werden gelöscht.",
        "cancel" to "Abbrechen",
        "settings" to "Einstellungen",
        "daily_goal" to "Tagesziel",
        "enter_goal" to "Ziel eingeben",
        "unit" to "Einheit",
        "bottle" to "\uD83C\uDF76 Flasche",
        "bottle_setup" to "Flascheneinstellung",
        "bottle_size_label" to "1 Flasche = ? oz",
        "bottle_hint" to "Schnell hinzufügen zeigt Flaschenbruchteile (¼, ½, ¾, 1).",
        "appearance" to "Darstellung",
        "dark_mode" to "Dunkelmodus",
        "language" to "Sprache",
        "save" to "Speichern",
        "history" to "Verlauf",
        "keep_it_going" to "Weiter so!",
        "recent_days" to "Letzte Tage",
        "goal_met" to "Ziel erreicht",
        "done" to "Fertig",
        "trends" to "Trends",
        "seven_days" to "7 Tage",
        "fourteen_days" to "14 Tage",
        "thirty_days" to "30 Tage",
        "average" to "Durchschnitt",
        "goals_met" to "Ziele erreicht",
        "streak" to "Serie",
        "daily_intake" to "Tägliche Aufnahme",
        "goal_line" to "Ziellinie",
        "back" to "Zurück",
        "bottles" to "Flaschen",
        "goal_colon" to "Ziel:"
    )

    private val pl = mapOf(
        "app_name" to "Water Camel",
        "goal_reached" to "Cel osiągnięty 🎉",
        "set_your_daily_goal" to "Ustaw dzienny cel",
        "set_goal_prompt" to "Stuknij ikonę koła zębatego, aby ustawić dzienny cel nawodnienia.",
        "set_goal" to "Ustaw cel",
        "day_streak_fmt" to "Seria %d dni!",
        "today" to "Dziś",
        "goal" to "Cel",
        "remaining" to "Pozostało",
        "todays_timing" to "Dzisiejszy rozkład",
        "morning" to "Rano",
        "afternoon" to "Popołudnie",
        "evening" to "Wieczór",
        "weekly_monthly_charts" to "📊 Wykresy tygodniowe i miesięczne",
        "quick_add" to "Szybkie dodawanie",
        "custom_amount" to "Własna ilość",
        "undo" to "Cofnij",
        "reset" to "Resetuj",
        "reset_today" to "Zresetować dzisiaj?",
        "reset_today_msg" to "To usunie całe dzisiejsze spożycie.",
        "cancel" to "Anuluj",
        "settings" to "Ustawienia",
        "daily_goal" to "Dzienny cel",
        "enter_goal" to "Wpisz cel",
        "unit" to "Jednostka",
        "bottle" to "\uD83C\uDF76 Butelka",
        "bottle_setup" to "Ustawienia butelki",
        "bottle_size_label" to "1 butelka = ? oz",
        "bottle_hint" to "Szybkie dodawanie pokaże ułamki butelki (¼, ½, ¾, 1).",
        "appearance" to "Wygląd",
        "dark_mode" to "Tryb ciemny",
        "language" to "Język",
        "save" to "Zapisz",
        "history" to "Historia",
        "keep_it_going" to "Tak trzymaj!",
        "recent_days" to "Ostatnie dni",
        "goal_met" to "Cel osiągnięty",
        "done" to "Gotowe",
        "trends" to "Trendy",
        "seven_days" to "7 dni",
        "fourteen_days" to "14 dni",
        "thirty_days" to "30 dni",
        "average" to "Średnia",
        "goals_met" to "Osiągnięte cele",
        "streak" to "Seria",
        "daily_intake" to "Dzienne spożycie",
        "goal_line" to "Linia celu",
        "back" to "Wstecz",
        "bottles" to "butelki",
        "goal_colon" to "Cel:"
    )

    private val pt = mapOf(
        "app_name" to "Water Camel",
        "goal_reached" to "Meta atingida 🎉",
        "set_your_daily_goal" to "Defina sua meta diária",
        "set_goal_prompt" to "Toque no ícone de engrenagem para definir uma meta diária de água.",
        "set_goal" to "Definir meta",
        "day_streak_fmt" to "Sequência de %d dias!",
        "today" to "Hoje",
        "goal" to "Meta",
        "remaining" to "Restante",
        "todays_timing" to "Horário de hoje",
        "morning" to "Manhã",
        "afternoon" to "Tarde",
        "evening" to "Noite",
        "weekly_monthly_charts" to "📊 Gráficos semanais e mensais",
        "quick_add" to "Adicionar rápido",
        "custom_amount" to "Quantidade personalizada",
        "undo" to "Desfazer",
        "reset" to "Redefinir",
        "reset_today" to "Redefinir hoje?",
        "reset_today_msg" to "Isso limpará toda a ingestão de hoje.",
        "cancel" to "Cancelar",
        "settings" to "Configurações",
        "daily_goal" to "Meta diária",
        "enter_goal" to "Insira a meta",
        "unit" to "Unidade",
        "bottle" to "\uD83C\uDF76 Garrafa",
        "bottle_setup" to "Configuração da garrafa",
        "bottle_size_label" to "1 garrafa = ? oz",
        "bottle_hint" to "Adicionar rápido mostrará frações da garrafa (¼, ½, ¾, 1).",
        "appearance" to "Aparência",
        "dark_mode" to "Modo escuro",
        "language" to "Idioma",
        "save" to "Salvar",
        "history" to "Histórico",
        "keep_it_going" to "Continue assim!",
        "recent_days" to "Dias recentes",
        "goal_met" to "Meta atingida",
        "done" to "Concluído",
        "trends" to "Tendências",
        "seven_days" to "7 dias",
        "fourteen_days" to "14 dias",
        "thirty_days" to "30 dias",
        "average" to "Média",
        "goals_met" to "Metas atingidas",
        "streak" to "Sequência",
        "daily_intake" to "Ingestão diária",
        "goal_line" to "Linha da meta",
        "back" to "Voltar",
        "bottles" to "garrafas",
        "goal_colon" to "Meta:"
    )
}
